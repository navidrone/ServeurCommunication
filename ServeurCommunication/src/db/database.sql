...
INSERT INTO `navidrone`.`COORD_GPS` (COORD_GPS_LATTITUDE,COORD_GPS_LONGITUDE) values (43.562777, 1.472315);
INSERT INTO `navidrone`.`COORD_GPS` (COORD_GPS_LATTITUDE,COORD_GPS_LONGITUDE) values (43.557811, 1.478758);
...
CREATE TABLE  `navidrone`.`MISSION` (
  ...
  INDEX `IDX_MISSION_ID` (`MISSION_ID`),	
    FOREIGN KEY (`COORD_GPS_DEP`) 
        REFERENCES `navidrone`.`COORD_GPS`(`COORD_GPS_ID`),	
    FOREIGN KEY (`COORD_GPS_AR`) 
        REFERENCES `navidrone`.`COORD_GPS`(`COORD_GPS_ID`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8;

INSERT INTO `navidrone`.`MISSION` (MISSION_NAME,COORD_GPS_DEP,COORD_GPS_AR) values ('MISSION_1',1,2);
INSERT INTO `navidrone`.`MISSION` (MISSION_NAME,COORD_GPS_DEP,COORD_GPS_AR,MISSION_TYPE,PORTEE,PERIODE,DENSITE) 
							values ('MISSION_2',1,2,'Bathymetrie',10,100,2);
